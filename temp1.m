...
for k = 1:size(BER_plot,1)
    ...
    if ~isempty(idx) && idx > 1
        ...
        xcross = interp1([y1 y2],[x1 x2],targetVal);

        BER_plot(k,idx:end) = NaN;

        BER_plot(k,idx-1) = targetVal;
...
BER_end_idx = zeros(1,size(BER_plot,1));
